...
from mc_mjlab.bridge.controller_objects import object_state
...
def test_object_layout_preserves_earlier_columns(count: int) -> None:
  layout = make_layout()
...
def test_object_pose_and_velocity_conventions() -> None:
  half = math.sqrt(0.5)
...
def test_object_feedback_reset_and_worker_recovery(
  tmp_path: Path, recover_worker: bool
) -> None:
  layout = make_layout()
