...
def sample(env, term, row):
  ...
  phases = [
    int(term.controller_scalar(f"Locomanip::{side}Phase")[row])
  ...
  return {
    ...
    "reference": term.controller_vector("Locomanip::objectReferencePosition")[
      row
    ].tolist(),
    "reference_rpy": term.controller_vector("Locomanip::objectReferenceRpy")[
      row
    ].tolist(),
    "phases": phases,
    "complete": bool(term.controller_scalar("Locomanip::complete")[row]),
    "contacts": contacts,
...
def result(initial, final, minimum_height, bilateral_steps, hold_since, dt):
