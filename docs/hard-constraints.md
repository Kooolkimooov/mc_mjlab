...
| 4 | 2026-09-14 | C2 | `placement_verdict`, kernel and guard roles, gate-conditioned quantiles in the audit | policy zero, 16 envs x 1000 steps: `recovery_dcm` admits 3200/16000 samples, gated median error **0.0411 m**, so `DCM_STD = 0.10` is **2.43x** | **OUT OF BAND** — floored side; `0.06` would give 1.46x. Change not made, pending a run |
